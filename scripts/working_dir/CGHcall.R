...
threeSampleNames = c("5-LD", "6-VJ", "8-MM")
osData = NULL
if(F) {
    ## for loading data from individual probeset.txt files
    for (sampleName in threeSampleNames) {
    ...
    colnames(osData) = c("probeID", "CHROMOSOME", "START_POS", "END_POS", threeSampleNames)
}
...
if(F) {
    ...
    warning("check samplenames order before processing this chunk.")
    ## for loading data from a single probeset.txt file that contains all samples data
    allSamplesPath = paste0(dataDir, "/allSamples_2_3_centeredProbeset.txt")
    ...
    ## add END_POS column
    ProbeData_filtered$END_POS = ProbeData_filtered$Position+20
...
main = function() {
    ...
    allSamplesClean_path = file.path(dataDir, "allSamplesCleanProbeset_2_3Rec.txt")
    # allSamplesClean_path = file.path(dataDir, "allSamplesCleanProbeset.txt") # where samples 2 and 3 are not centered
    ...
    ## plotting 3 samples within the same plot
    ...
    ## initialize GI df
    GI_CGHcall_df = data.frame(matrix(ncol = 4, nrow = 0))
    colnames(GI_CGHcall_df) = c("sample","GI_CGHcall", "nbAlter_CGHcall", "nbChr_CGHcall")
    # GI_CGHcall_df = data.frame(rep(NA, length(sampleNames)))
    # colnames(GI_CGHcall_df) = "GI_CGHcall"
    ...
    for (s in 1:length(allSegTables)) {
        ...
        GI_CGHcall_df[s,] = append(list(sampleNames[s]), GI_res)
        # print(GI_res)
    }
    ## saving GI table
