...
plotData = callData
# remove "chr" so segments$chr matches hg38 notation
plotData$segments$chr = lapply(plotData$segments$chr,stringr::str_replace, "chr", "")
# remove Y chromosome data
plotData$segments = dplyr::filter(plotData$segments,chr!="Y")
ascat.plotAdjustedAscatProfile(plotData,REF="hg38",png_prefix=outputFolder_plots)
## plot logR and BAF raw Data
ascat.plotRawData(segData,img.dir=outputFolder_plots)
## plot logR and BAF data before vs after segmentation
ascat.plotSegmentedData(segData,img.dir=outputFolder_plots)
