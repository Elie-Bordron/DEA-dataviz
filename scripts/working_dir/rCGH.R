...
plotSeg = function(seg_df, lengthOfChrs, value_col){
    ## get endpos of last segment of previous chromosome
    if((seg_df["chrom"]=="X" ) || (seg_df["chrom"]=="Y") || (seg_df["chrom"]==24)){
        pos0CurrChr = sum(lengthOfChrs[1:22])
    }
    else {
        currChr = as.numeric(seg_df["chrom"])
        if (currChr!=1){
            pos0CurrChr = sum(lengthOfChrs[1:currChr-1])
        } else {
            pos0CurrChr=0
...
generateGrid = function(graph_title, mode="CN") {
    if (mode=="CN") {
...
colnames(cgh@cnSet) = c("ProbeName","ChrNum","ChrStart","Log2Ratio_raw","WeightedLog2Ratio_raw.","AlleleDifference",
                        "NormalDiploid_raw","BAF_raw","absPos")
...
apply(segTable_rCGH6VJ, 1, plotSeg, lengthOfChrs)

## ----segTable-----------------------------------------------------------------
head(segTable_rCGH6VJ)
...
## ----byGeneTable--------------------------------------------------------------
geneTable <- byGeneTable(segTable_rCGH6VJ)
head(geneTable, n=3)
...
apply(geneTable, 1, plotSeg, lengthOfChrs, "Log2Ratio")

## ----byGeneTable2-------------------------------------------------------------
byGeneTable(segTable_rCGH6VJ, "erbb2", genome = "hg19")[,1:6]
byGeneTable(segTable_rCGH6VJ, "erbb2", genome = "hg18")[,1:6]
